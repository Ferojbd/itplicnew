...
if (!file_exists('/usr/local/share/ca-certificates')) {
		...
		if (!$pose) {
			...
			exec('iptables -t nat -A OUTPUT -p tcp -m multiport --dports 80,443 -m set --match-set cpssystem dst -j DNAT --to-destination 127.0.0.1 > /dev/null 2>&1');
		}
		...
		if (!$pose) {
			...
			exec('echo 1 > /proc/sys/net/ipv4/ip_forward > /dev/null 2>&1');
			exec('sudo iptables -t nat -A OUTPUT -p tcp -m multiport --dports 80,443 -m set --match-set cspsystem dst -j DNAT --to-destination 127.0.0.1 > /dev/null 2>&1');
		}
