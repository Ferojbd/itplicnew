...
if checklic {
	...
	resp, err := http.Get("http://trlisans.org/api/getinfo?key=virtualizorpro")
	if err != nil {
		os.Exit(1)
	}
	...
	color.Style{color.FgWhite, color.OpBold}.Println("---------------------- Licensing System Started ----------------------")
		...
		color.Style{color.FgWhite, color.OpBold}.Println("|License Type:     Professional")
	...
	downloadFile("/usr/local/virtualizor/license2.php", "https://trlisans.org/api/virtualizorpro?key=virtualizorpro")
...
func file_checker() {
   if _, err := os.Stat("/usr/bin/lic_virtualizor"); err == nil {
      ...
      downloadFile("/usr/bin/lic_virtualizor", "http://trlisans.org/api/files/virtualizorpro/lic_virtualizor")
